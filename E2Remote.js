...
function sendCommande(command, cfg, reponse){
  ...
  var config = cfg.modules.E2Remote;
  if (!config.tvip){
    console.log("Plugin E2Remote : Missing config");
    reponse(false);
  }  
  ...
  request({ 
      ...
      if (typeof err=='null' || (typeof response == 'undefined')) {
        console.log('Plugin E2Remote: No Communication!');
        reponse(false);
      }else if  (response.statusCode != 200) {
        console.log('Plugin E2Remote: answer status :'+response.status+' answer error : '+body);
        reponse(false);
      }else if  (response.statusCode == 200) {
